...
async def coupon_add(
    ...
    new_coupon = await coupon_service.add(coupon_dto, current_user.user.id, current_user.tenant.id)
...
async def get_admin_coupons(
    ...
    coupon_repository = SQLCouponRepository(db)
    coupon_service = CouponUseCases(coupon_repository)
    coupons = await coupon_service.get_all_admin(current_user.tenant.id)
    return {
        "error_message": None,
        "success": True,
        "error_code": None,
...
async def get_admin_coupon(
    ...
    coupon_repository = SQLCouponRepository(db)
    coupon_service = CouponUseCases(coupon_repository)
    coupon = await coupon_service.get_by_id_admin(id, current_user.tenant.id)
    return {
        "error_message": None,
        "success": True,
        "error_code": None,
...
async def admin_coupon_update(
    ...
    coupon_repository = SQLCouponRepository(db)
    coupon_service = CouponUseCases(coupon_repository)
    coupon = await coupon_service.update_coupon_admin(id, coupon_update_dto, current_user.user.id, current_user.tenant.id)
    if coupon is False:
...
async def remove_admin_coupon(
    ...
    coupon_repository = SQLCouponRepository(db)
    coupon_service = CouponUseCases(coupon_repository)
    coupon = await coupon_service.delete_coupon_admin(id, current_user.user.id, current_user.tenant.id)
    if coupon is False:
