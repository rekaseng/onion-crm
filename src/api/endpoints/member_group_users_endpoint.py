...
async def member_group_users_add(
    ...
    new_member_group_users = await member_group_users_service.add(member_group_users_dto, current_user)
...
async def get_member_group_users(
        current_user: dict = Depends(get_current_user),
    ...
    member_group_users_repository = SQLMemberGroupUsersRepository(db)
    member_group_users_service = MemberGroupUsersUseCases(member_group_users_repository)
    member_group_users = await member_group_users_service.get_all(current_user)
    return {
        "error_message": None,
        "success": True,
        "error_code": None,
...
async def get_member_group_users(
    ...
    member_group_users_repository = SQLMemberGroupUsersRepository(db)
    member_group_users_service = MemberGroupUsersUseCases(member_group_users_repository)
    member_group_users = await member_group_users_service.get_by_user_id(user_id, current_user)
    return {
        "error_message": None,
        "success": True,
        "error_code": None,
...
async def member_group_users_update(
    ...
    member_group_users_repository = SQLMemberGroupUsersRepository(db)
    member_group_users_service = MemberGroupUsersUseCases(member_group_users_repository)
    member_group_user = await member_group_users_service.update_member_group_users(member_group_users_update_dto, current_user)
...
async def remove_member_group_users(
        user_id: int,
        current_user: dict = Depends(get_current_user),
        permission: bool = Depends(check_permission("member_groups_write")),
        db: AsyncSession = Depends(get_db)
    ...
    member_group_users_repository = SQLMemberGroupUsersRepository(db)
    member_group_users_service = MemberGroupUsersUseCases(member_group_users_repository)
    member_group_user = await member_group_users_service.delete_member_group_users(user_id, current_user)
    if member_group_user is False:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Member Group User Delete Unsuccessful")
    return {
        "error_message": None,
        "success": True,
        "error_code": None,
